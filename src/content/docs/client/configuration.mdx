...
This is the **read** API key from TMDB to allow P-Stream to search for media. [Get one by following our guide](/smov-docs/client/tmdb).
...
This is where you put proxy URLs. [Get some by following our guides](/smov-docs/proxy/deploy).
...
**The same proxy as above**, but used to proxy streams. However, it **must** be hosted on a server that is NOT blocked by FlixHQ. See this [explainer](/smov-docs/proxy/introduction).
...
[The proxy workers will need to be configured to accept these captcha tokens](/smov-docs/proxy/configuration#turnstile-secret), otherwise it has no effect for security.
