...
Please read [THIS](/smov-docs/proxy/introduction) and use Method 1 (Docker) or Method 2 (Manual)!
...
5. Run the Docker container (replace values as needed):
   `docker run -d --name p-stream-proxy -p 3000:3000 --env-file .env p-stream-proxy`
...
<Steps>
1. Clone the repo on your server `git clone https://github.com/p-stream/simple-proxy`
...
8. Use the URL you took note of and [set up a custom proxy in the
   client](/smov-docs/extra/streaming#method-1-self-hosted-proxy).
</Steps>
...
6. Find the link of your site near the top of the page, it should look something like `https://<random-words>.netlify.app`. Right click the link, click `Copy link address`. [Now you'll just have to set up the client](/smov-docs/extra/streaming#method-1-self-hosted-proxy)!
</Steps>
...
6. Congratulations! You have deployed the proxy, [you can now set up the client](/smov-docs/extra/streaming#method-1-self-hosted-proxy).
</Steps>
